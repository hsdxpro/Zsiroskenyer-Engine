	...
	if(it == collisionShapes.end()) {
		...
		if (mass == 0) {
			...
			for (size_t i = 0; i < d.nVertices; i++) {
				memcpy(vertices[i], (unsigned char*)d.vertices + i * d.vertexFormat.GetByteSize(), sizeof(Vec3));
			}
			...
			colShape = new btConvexHullShape((btScalar*)d.vertices, d.nVertices, d.vertexFormat.GetByteSize());
		}
...
IPhysicsEntity* cPhysicsEngineBullet::CreateSoftEntity(const zsString& physicsGeom, float mass)  {
	...
	for (size_t i = 0; i < d.nVertices; i++)
		memcpy(&vertices[i * 3], (unsigned char*)d.vertices + i * d.vertexFormat.GetByteSize(), sizeof(btScalar) * 3);
